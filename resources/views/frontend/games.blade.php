...
<div class="cs-slider cs-style1">
  ...
  <div class="container">
    <div class="cs-seciton_heading cs-style4 text-center">
      <h2 class="cs-seciton_title">Gaming Products</h2>
    </div>
    <div class="cs-height_30 cs-height_lg_30"></div>
    <div class="row cs-row_gap_10">
      @if($categories)
        @foreach($categories as $key=>$cat)
          <div class="col-xl-3 col-md-4 col-sm-6">
            <a href="{{route('front.categoryview',$cat->id)}}" class="cs-category cs-style2
            @if($key==0) cs-color1 
            @elseif($key==1) cs-color2
            @elseif($key==2) cs-color3
            @elseif($key==3) cs-color4
            @elseif($key==4) cs-color5
            @elseif($key==5) cs-color6
            @elseif($key==6) cs-color7
            @elseif($key==7) cs-color8
            @else  cs-color1 @endif">
              <h2 class="category_title">{{$cat->name}}</h2>
              <div class="cs-category_img"><img src="{{asset($cat->cat_image)}}" alt=""></div>
            </a>
  ...
  <div class="container cs-card_wrap_1">
    ...
    <div class="cs-card_grip_2">
      <div class="cs-card_out">
        <a href="{{$gamsetting->mp_add_link}}" class="cs-brand"><img src="@if($gamsetting->mp_add) {{asset($gamsetting->mp_add)}} @else {{asset('assets/img/gaming/brand1.jpg')}} @endif" alt=""></a>
      </div>
      @php
        $products=App\Models\Product::where('popular',1)->whereIn('category_id',$category)->take(11)->latest()->get();
      @endphp
      @if($products)
  ...
  <div class="container cs-card_wrap_1">
    ...
    <div class="cs-card_grip_2">
      <div class="cs-card_out">
        <a href="{{$gamsetting->fp_add_link}}" class="cs-brand"><img src="@if($gamsetting->fp_add) {{asset($gamsetting->fp_add)}} @else {{asset('assets/img/gaming/brand2.jpg')}} @endif" alt=""></a>
      </div>
        @php
          $products=App\Models\Product::where('feature_product',1)->whereIn('category_id',$category)->take(11)->latest()->get();
        @endphp
        @if($products)
          @foreach ($products as $item)
            <div class="cs-card_out">
  ...
  <div class="container">
    <div class="row cs-card_wrap">
      <div class="col-lg-3">
        <a href="{{$gamsetting->bs_add_link}}" class="cs-brand"><img src="@if($gamsetting->bs_add) {{asset($gamsetting->bs_add)}} @else {{asset('assets/img/gaming/brand3.jpg')}} @endif" alt=""></a>
      </div>
      <div class="col-lg-9">
            ...
            <div id="tab_one" class="cs-tab active">
              <div class="cs-slider cs-style3 cs-gap-10">
                <div class="cs-slider_container" data-autoplay="0" data-loop="1" data-speed="600" data-center="0" data-slides-per-view="responsive" data-xs-slides="1" data-sm-slides="2" data-md-slides="3" data-lg-slides="4" data-add-slides="5">
                  <div class="cs-slider_wrapper">
                    @php
                      $products=App\Models\Product::where('best_seller',1)->where('new_product',1)->whereIn('category_id',$category)->orderBy('id','DESC')->paginate(8);
                    @endphp
                    @if($products)
                      @foreach ($products as $item)
                        <div class="cs-slide">
            ...
            <div id="tab_two" class="cs-tab">
              <div class="cs-slider cs-style3 cs-gap-10">
                <div class="cs-slider_container" data-autoplay="0" data-loop="1" data-speed="600" data-center="0" data-slides-per-view="responsive" data-xs-slides="1" data-sm-slides="2" data-md-slides="3" data-lg-slides="4" data-add-slides="5">
                  <div class="cs-slider_wrapper">
                  @php
                      $products=App\Models\Product::where('best_seller',1)->whereIn('category_id',$category)->inRandomOrder()->paginate(8);
                    @endphp
                    @if($products)
                      @foreach ($products as $item)
                        <div class="cs-slide">
